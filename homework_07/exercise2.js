...
exports.getDecryptedMessage = (callback) => {
	...
	const collection = state.db.collection('mwa_homework_07');
	const message = collection.findOne({_id:10}, {_id: 0});

	message.then(data => {
		const decipher = crypto.createDecipher(algorithm, pass);
		const decrypted = decipher.update(data.data, 'hex', 'utf8') + decipher.final('utf8');
